...
function resolveVaultId(getSettings: () => AppSettings, vaultId?: string): string {
  const settings = getSettings();
  return (
    settings.vaults.find((vault) => vault.id === vaultId)?.id ??
    settings.vaults.find((vault) => vault.id === settings.activeVaultId)?.id ??
    settings.vaults[0]?.id ??
    ""
  );
}
...
export function registerChatIpc(options: {
  ...
  ipcMain.handle(ipcChannels.chatStoreMemory, async (_event, input: unknown) => {
    ...
    const vaultId = resolveVaultId(options.getSettings, parsed.vaultId);
