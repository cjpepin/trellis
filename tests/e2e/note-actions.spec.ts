...
test("reviews proposed wiki note actions from chat", async ({ page }) => {
  ...
  const seeded = await page.evaluate(async (commonDraft: string) => {
    ...
    await window.trellis.db.replaceMessages({
      ...
      messages: [
        ...
        },
        {
          ...
          sessionId: session.id,
          role: "user",
          content: "Save those as new wiki notes.",
          createdAt: now + 1,
          tokens: null
        },
        {
          ...
          sessionId: session.id,
          role: "assistant",
          content: "I prepared two note drafts for review.",
          createdAt: now + 2,
          tokens: null,
          noteActions: [
            {
              ...
              rationale: "Save the rejected note draft.",
            ...
            },
            {
              ...
              rationale: "Save the approved note draft.",
              sourceMessageIds: [draftedMessageId, saveRequestId],
              createdAt: now + 2
            }
          ]
    ...
    return {
      approvedPath: `${vault.path}/wiki/${approvedSlug}.md`,
      rejectedPath: `${vault.path}/wiki/${rejectedSlug}.md`
    };
  }, noteReviewDraft);
