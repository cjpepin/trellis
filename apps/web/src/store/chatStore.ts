...
import type { AppWorkspaceId, ChatSessionSummary, MessageRecord } from "@trellis/contracts";
import {
  canStartChatRun,
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  removeSession: (sessionId) =>
    set((state) => {
      ...
      const nextActiveSessionId = wasActive
        ? sessions[0]?.id ?? null
        : state.activeSessionId;

      const { [sessionId]: _removedMsgs, ...nextMessagesBySession } = state.messagesBySession;
      const { [sessionId]: _removedRun, ...nextChatRuns } = state.chatRunsBySession;
      ...
      return {
        ...
        chatRunsBySession: nextChatRuns,
        chatRunNotificationsBySession: Object.fromEntries(
          Object.entries(state.chatRunNotificationsBySession).filter(([id]) => id !== sessionId)
        ),
        lastExtractedMessageCount: Object.fromEntries(
          Object.entries(state.lastExtractedMessageCount).filter(([id]) => id !== sessionId)
        )
      };
    }),
  addMessage: (message) =>
