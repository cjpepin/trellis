...
import type { StrandRevisionActor, StrandRevisionSummary } from "@trellis/contracts";
...
import { isCloudBackedVaultActive } from "@/lib/cloud/bucket";
...
function wikiSlugFromRelativePath(relativePath: string): string {
...
export function StrandRevisionHistory({ bucketId, file, open, onClose }: Props): JSX.Element | null {
  ...
  useEffect(() => {
    ...
    if (isCloudBackedVaultActive() && cloudWorkspaceId) {
      const slug = wikiSlugFromRelativePath(file);
      void getTrellisApiClient()
        .listNoteRevisions(cloudWorkspaceId, slug)
        .then((list) => {
          if (!cancelled) {
            const mapped: StrandRevisionSummary[] = list.map((row) => ({
              id: row.id,
              bucketId,
              file: row.file,
    ...
    void window.trellis.db
      .listStrandRevisions({ bucketId, file })
      .then((list) => {
        if (!cancelled) {
          setRows(list);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const cloudWorkspaceId = getActiveCloudWorkspaceRuntime()?.cloudWorkspaceId ?? null;
        const fetchBody = async (revisionId: string): Promise<{ body: string } | null> => {
          if (isCloudBackedVaultActive() && cloudWorkspaceId) {
            return getTrellisApiClient().getNoteRevisionBody(cloudWorkspaceId, revisionId);
          }
          return window.trellis.db.getStrandRevisionBody({ bucketId, revisionId });
        };

        const newerRow = await fetchBody(newerId);
        ...
        let before = "";
        if (olderId) {
          const olderRow = await fetchBody(olderId);
