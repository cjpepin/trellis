...
import { transcribeAudioBridged } from "@/lib/cloud/chatMediaBridge";
      ...
      recorder.onstop = () => {
        ...
        void (async () => {
          ...
          try {
            ...
            try {
              const result = hasElectronPreloadBridge()
                ? await window.trellis.media.transcribe({
                    accessToken: accessToken ?? "",
                    subscriptionTier,
                    audioBase64,
                    mimeType: blob.type || "audio/webm"
                  })
                : await transcribeAudioBridged({
                    accessToken: accessToken ?? "",
                    subscriptionTier,
                    audioBase64,
                    mimeType: blob.type || "audio/webm",
