...
import type { AppSettings, ThoughtRecord } from "@trellis/contracts";
...
import {
  createThoughtBridged,
  getThoughtBridged,
  retryThoughtEnrichmentBridged
} from "@/lib/cloud/thoughts";
import { getActiveCloudWorkspaceRuntime } from "@/lib/cloud/runtime";
import { writeBucketNote } from "@/lib/cloud/bucket";
...
export function Thoughts({ settings }: Props) {
  ...
  const refreshThought = useCallback(
    async (id: string) => {
      const latest = await getThoughtBridged(id, vault.id);

      if (latest) {
        upsertThought(latest);
      }
    },
    [upsertThought, vault.id]
  );
  ...
  async function handleSubmit(event: FormEvent): Promise<void> {
    ...
    try {
      const created = await createThoughtBridged({
        bucketId: vault.id,
        content: text,
        sourceType: "manual"
      });
  ...
  async function handleRetryEnrichment(): Promise<void> {
    ...
    try {
      await retryThoughtEnrichmentBridged(selected.id);
      await refreshThought(selected.id);
      pushToast({
        title: getActiveCloudWorkspaceRuntime()?.cloudWorkspaceId
          ? "Enrichment finished"
          : "Retry scheduled",
        tone: "success"
      });
    ...
    <div className="flex h-full min-h-0 flex-col gap-6 p-6" data-testid="route-thoughts">
      ...
      <form onSubmit={(event) => void handleSubmit(event)} className="shrink-0 space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <span className="text-xs text-trellis-faint">
            {getActiveCloudWorkspaceRuntime()?.cloudWorkspaceId
              ? "Saved to your workspace · linking runs in the background"
              : "Saved locally · enrichment runs in the background"}
          </span>
        </div>
      </form>
