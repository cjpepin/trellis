...
export function Settings({
  ...
  async function runManualSync(): Promise<void> {
    ...
    try {
      const result = await window.trellis.retrieval.rebuildIndex(activeBucket.id);
      pushToast({
        title: result.usedEmbeddings
          ? `Synced ${result.notesIndexed} notes for ${activeBucket.name}.`
          : `Synced ${result.notesIndexed} notes for ${activeBucket.name} without embeddings.`,
        tone: "success"
      });
    } catch (error) {
      pushToast({
        title: error instanceof Error ? error.message : "Could not sync the bucket search index.",
        tone: "warning"
      });
    } finally {
  ...
  async function openBucket(targetPath: string): Promise<void> {
    try {
      await window.trellis.shell.openPath(targetPath);
    } catch (error) {
      pushToast({
        title: error instanceof Error ? error.message : "Could not open that folder.",
        tone: "warning"
      });
    }
  }

  async function addBucket(): Promise<void> {
    const normalizedName = newBucketName.trim();

    if (!normalizedName) {
      pushToast({
        title: "Name the bucket before choosing its folder.",
        tone: "warning"
      });
      return;
    }
    ...
    try {
      const selectedPath = await window.trellis.bucket.selectDirectory({
        title: "Choose your Trellis bucket folder",
      ...
      if (settings.buckets.some((bucket) => bucket.path === selectedPath)) {
        throw new Error("That folder is already in your bucket list.");
      }
      ...
      await onUpdateSettings({
        ...settings,
        buckets: [...settings.buckets, nextBucket],
        activeBucketId: nextBucket.id
      });
      setNewBucketName("");
      ...
      pushToast({
        title: error instanceof Error ? error.message : "Could not add that bucket.",
        tone: "error"
      });
    }
  }

  async function activateBucket(bucketId: string): Promise<void> {
    try {
      await onUpdateSettings({
        ...settings,
      ...
      pushToast({
        title: error instanceof Error ? error.message : "Could not switch buckets.",
        tone: "error"
      });
    }
  }
  ...
  async function importFromObsidian(): Promise<void> {
    ...
    try {
      const sourcePath = await window.trellis.bucket.selectDirectory({
      ...
      const result = await window.trellis.bucket.importFromObsidian({
        sourcePath,
        bucketId: activeBucket.id
      });

      await onRefreshBucket(activeBucket.id);
      pushToast({
        title: `Imported ${result.importedNoteCount} Obsidian notes into ${activeBucket.name}.`,
        tone: "success"
      });
    } catch (error) {
      pushToast({
  ...
  async function exportToObsidian(): Promise<void> {
    ...
    try {
      const targetPath = await window.trellis.bucket.selectDirectory({
      ...
      const result = await window.trellis.bucket.exportToObsidian({
        targetPath,
        bucketId: activeBucket.id
      });
        ...
        <div className="grid gap-4 lg:grid-cols-2 lg:items-start">
          <div className="trellis-panel flex min-h-0 flex-col px-4 py-3">
            <div className="flex flex-wrap items-start justify-between gap-2">
              <div className="min-w-0">
                <p className="text-sm font-medium text-trellis-text">Buckets</p>
                <p className="mt-0.5 text-[11px] leading-snug text-trellis-muted">
                  Separate buckets for projects, clients, or research threads.
                </p>
              </div>
              <button
                type="button"
                ...
                onClick={() => {
                  void openBucket(activeBucket.path);
                }}
              >
            ...
            <div className="mt-2 min-h-0 space-y-1.5">
              {settings.buckets.map((bucket) => {
                const isActive = bucket.id === settings.activeBucketId;
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <div className="min-w-0">
                        <div className="flex items-center gap-1.5">
                          <p className="text-xs font-medium text-trellis-text">{bucket.name}</p>
                          {isActive && (
                        ...
                        <p className="mt-0.5 truncate text-xs text-trellis-muted">{bucket.path}</p>
                      </div>
            ...
            {isAddingBucket ? (
              <div className="mt-2 grid gap-2 rounded-panel border border-dashed border-trellis-border px-2.5 py-2">
                <div className="flex items-center justify-between gap-2">
                  <p className="text-[11px] font-medium text-trellis-text">New bucket</p>
                  <button
                    type="button"
            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div className="min-w-0 max-w-md">
                  ...
                  <p className="mt-1 text-[11px] leading-relaxed text-trellis-muted">
                    Bring markdown notes from an Obsidian vault into {activeBucket.name}, or export
                    this Trellis bucket back into an Obsidian-friendly folder.
                  </p>
                </div>
              ...
              <p className="mt-3 text-[11px] leading-relaxed text-trellis-muted">
                ...
                inside your active Trellis bucket. Exports write into{" "}
                <span className="font-mono text-trellis-text">Trellis/{activeBucket.name}</span> in the
          ...
          <div className="flex min-w-0 flex-col gap-4">
            ...
            <label className="block" htmlFor="settings-chat-privacy-mode">
              ...
              <span className="mt-0.5 block text-[11px] leading-snug text-trellis-muted">
                Keep local bucket retrieval and choose how much context Trellis may send with a chat reply.
              </span>
            </label>
            <ListboxSelect
          ...
          <label className="block" htmlFor="settings-notes-from-chats-mode">
            ...
            <span className="mt-0.5 block text-[11px] leading-snug text-trellis-muted">
              Linked notes from chat into your bucket.
            </span>
          </label>
              ...
              <div className="rounded-panel border border-trellis-border bg-trellis-surface-2/80 px-3 py-3">
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div className="min-w-0 max-w-2xl">
                    ...
                    <p className="mt-0.5 text-[11px] leading-relaxed text-trellis-muted">
                      Refresh the active bucket’s search index or force note processing for{" "}
                ...
                <div className="mt-3 grid gap-2 xl:grid-cols-2">
                  <div className="rounded-panel border border-trellis-border bg-trellis-surface px-3 py-3">
                    ...
                    <p className="mt-1 text-[11px] leading-relaxed text-trellis-muted">
                      Rebuild the retrieval index for {activeBucket.name}.
                    </p>
                    <button
                      type="button"
        ...
        <div className="trellis-panel px-4 py-3">
          ...
          {authState.subscriptionTier !== "byok" ? (
            <p className="mt-2 text-sm leading-6 text-trellis-muted">
              ...
              while Trellis keeps your bucket notes, local history, and app experience intact.
            </p>
          ) : (
            <>
              <p className="mt-2 text-sm leading-6 text-trellis-muted">
