...
import { getChatModelMediaCapabilities } from "@trellis/shared/chat/capabilities";
import { messageLikelyExpectsBucketContextForChat } from "@trellis/shared/chat/privacyBucketIntent";
import { buildChatReplyContext } from "@trellis/shared/chat/replyContext";
...
import { ChatBucketSelect } from "@/components/chat/ChatBucketSelect";
...
export function Chat({
  ...
  const flushComposerSourceIngest = useCallback(
    async (drafts: IngestedDraft[], sessionId: string | null, bucketId: string) => {
      if (drafts.length === 0) {
        return;
      }
      ...
      for (const draft of drafts) {
        try {
          ...
          await applyExtractionComposer(response, { sessionId: sessionId ?? undefined, bucketId });
        } catch (error) {
          pushToast({
            title:
              error instanceof Error
  ...
  const loadNote = useCallback(
    async (slug: string, bucketId: string) => {
      ...
      const note = await window.trellis.bucket.readNote(slug, bucketId);
      setNote(note);
      return note;
    },
    [setNote]
  ...
  const openReferencedNote = useCallback(
    async (slug: string) => {
      const bucketId = activeSession?.bucketId || activeBucket.id;

      if (!notes.some((note) => note.slug === slug)) {
        pushToast({
          title: "That Strand is not available in this bucket yet.",
          tone: "warning"
        });
        return;
      }
    ...
    [activeSession?.bucketId, activeBucket.id, loadNote, navigate, notes, pushToast, setActiveNote]
  );
  ...
  async function sendMessage(
    ...
    const sessionBucketId = activeSession?.bucketId || activeBucket.id;
    ...
    try {
        ...
        try {
          ...
          if (isChatNoteActionReview || proposal.clarification) {
            const defaultReviewCopy =
              "Here’s a proposed wiki note change. Review the diff and approve to save it to your bucket.";
      ...
      if (settings.chat.privacyMode === "off") {
        ...
        const likelyWantsBucket =
          messageLikelyExpectsBucketContextForChat(userTextForIntent) || pinnedWikiNotes.length > 0;

        if (likelyWantsBucket) {
          pushToast({
            title:
              "Chat privacy is Off, so wiki excerpts and pinned notes are not sent to the cloud model. Replies may miss your bucket context. Set Chat privacy to Auto or Local in Settings when you want answers grounded in your notes.",
            tone: "warning",
            durationMs: 5200
          });
        }
            ...
            persistedMessages = nextMessages.map((message) =>
              message.id === streamResult.assistantMessage?.id
                ? {
                    ...message,
                    content:
                      "I drafted a wiki note update and queued it for review. Make any final edits below, then approve to save it to your bucket.",
      ...
      void window.trellis.chat
        ...
        .then(async (organizeResult) => {
          ...
          try {
            const snapshot = await window.trellis.bucket.listIndex(sessionBucketId);
            replaceWikiIndex({
      ...
      if (ingestDrafts.length > 0 && sessionId && optimisticUserMessage) {
        void flushComposerSourceIngest(ingestDrafts, sessionId, sessionBucketId);
      }
  ...
  async function handleApproveNoteAction(messageId: string, actionId: string): Promise<void> {
    ...
    try {
      const result = await window.trellis.bucket.writeNote({
        bucketId,
      ...
      const snapshot = await window.trellis.bucket.listIndex(bucketId);
      setNote(result.note);
      replaceWikiIndex({
        notes: snapshot.notes,
        folders: snapshot.folders,
  ...
  async function handleAddBucket(): Promise<void> {
    const entered = window.prompt("Name for the new bucket", "");
    ...
    if (!trimmed) {
      pushToast({
        title: "Enter a name for the bucket.",
        tone: "warning"
      });
      return;
    }
    ...
    try {
      ...
      if (settings.buckets.some((bucket) => bucket.path === selectedPath)) {
        pushToast({
          title: "That folder is already in your bucket list.",
          tone: "warning"
        });
        return;
      }
    ...
    <div className="flex h-full flex-col" data-testid="route-chat">
      ...
      <div className="flex min-h-0 flex-1 flex-col lg:flex-row">
        ...
        <MessageList
          ...
          bucketId={activeSession?.bucketId ?? activeBucket.id}
          notes={notes}
          ...
          <div className="flex w-full shrink-0 flex-col gap-2 self-end sm:w-auto">
            {!activeSessionId && (
              <ChatBucketSelect
                ...
                activeBucketId={settings.activeBucketId}
                disabled={chatDisabled}
        ...
        {sessions.length > 0 && currentMessages.length > 0 && (
          ...
          >
            <p className="min-w-0">
              {activeSession && (
                <>
                  This conversation is writing to {getBucketById(settings, activeSession.bucketId).name}.{" "}
                </>
              )}
