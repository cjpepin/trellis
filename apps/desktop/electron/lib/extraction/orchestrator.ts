...
import { prepareExtractionWrite, skipIfDuplicatePreparedExtractionContent } from "@trellis/contracts/extraction/guardrails";
import {
...
import { extractionJobRelatedNotesLimit } from "@trellis/shared/extraction/config";
import {
...
import { oneHopWikiNeighborSlugs, pickSeedsForGraphExpansion } from "@trellis/shared/chat/wikiGraph";
import {
  EXTRACTION_GRAPH_NEIGHBOR_LIMIT,
  EXTRACTION_GRAPH_SEED_LIMIT,
  EXTRACTION_VAULT_RECENT_SLUGS_LIMIT
  ...
  async function processJob(jobId: string): Promise<void> {
    ...
    try {
      logExtraction("processJob.start", {
        ...
        bucketId: job.bucketId.slice(0, 8),
        trigger: job.trigger,
      ...
      const vault = resolveBucket(options.getSettings(), session.bucketId);
      const jobMode = resolveExtractionMode(session.model);
      const isCloud = jobMode === "cloud";

      const [storedConfig, snapshot, priorSessionSlugs, recentVaultSlugs] = await Promise.all([
        getExtractionJobConfig(job.id),
        buildSnapshot(vault.path, vault.id, vault.name),
        getSessionNoteSlugs(job.sessionId),
        listRecentVaultTouchedSlugs(vault.id, EXTRACTION_VAULT_RECENT_SLUGS_LIMIT)
      ]);
      ...
      const validSlugsForGraph = new Set(snapshot.notes.map((note) => note.slug));
      const graphSeeds = pickSeedsForGraphExpansion({
        explicitSlugs: [...new Set([...priorSessionSlugs, ...bracketSlugs])],
        recentSlugs: recentVaultSlugs,
        topRetrievalSlugs: [],
        maxSeeds: EXTRACTION_GRAPH_SEED_LIMIT
      });
      const graphNeighborSlugs = oneHopWikiNeighborSlugs(
        graphSeeds,
        snapshot.graph.edges.map((edge) => ({ source: edge.source, target: edge.target })),
        { validSlugs: validSlugsForGraph, maxResults: EXTRACTION_GRAPH_NEIGHBOR_LIMIT }
      );
      const explicitSlugs = [
        ...new Set([...priorSessionSlugs, ...bracketSlugs, ...recentVaultSlugs, ...graphNeighborSlugs])
      ];
      ...
      logExtraction("processJob.retrieval", {
        ...
        recentVaultSlugCount: recentVaultSlugs.length,
        graphNeighborSlugCount: graphNeighborSlugs.length,
