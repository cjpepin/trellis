...
import { readNoteIfExists } from "../../ipc/bucket";
...
/**
 * Merges embedding-based retrieval with title and query lexical matches so secondary topics
...
export async function mergeRelatedNotesWithLexicalAugmentation(
  ...
  for (const { note, score } of candidates.slice(0, lexicalSlots)) {
    const full = await readNoteIfExists(bucketPath, note.slug);
    if (!full) {
      continue;
    }
