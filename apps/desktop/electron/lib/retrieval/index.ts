...
export async function syncNoteEmbeddings(
  ...
  if (chunks.length === 0) {
    await replaceNoteEmbeddings(bucketId, note.slug, []);
    return {
      chunkCount: 0,
      embeddingModel: null,
...
export async function rebuildBucketEmbeddings(
  bucketId: string,
  notes: WikiNote[]
  ...
  for (const note of notes) {
    const result = await syncNoteEmbeddings(bucketId, note);
...
export async function searchRelevantNotes(
  ...
  let rows = await listNoteEmbeddings(input.bucketId);
