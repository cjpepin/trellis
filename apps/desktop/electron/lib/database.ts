...
interface ExtractionJobRow {
  ...
  /** Legacy rows may store `"auto"` or `"cloud"`; snapshots always report `"local"`. */
  mode: string;
...
export async function createSession(model: ChatModel, bucketId: string): Promise<ChatSessionSummary> {
  const db = getDatabase();
  const now = Date.now();
  ...
  runExec(
    ...
    `INSERT INTO chat_sessions (id, title, created_at, updated_at, model, vault_id, message_count)
     VALUES (?, ?, ?, ?, ?, ?, 0)`,
    [id, "Untitled Session", now, now, model, bucketId]
  );
...
/** Permanently remove a chat session and its messages from the local SQLite store. */
export async function deleteSession(sessionId: string): Promise<void> {
  const db = getDatabase();
  const txn = db.transaction(() => {
    runExec(db, `DELETE FROM messages WHERE session_id = ?`, [sessionId]);
    runExec(db, `DELETE FROM extraction_jobs WHERE session_id = ?`, [sessionId]);
    runExec(db, `DELETE FROM wiki_ops WHERE session_id = ?`, [sessionId]);
    runExec(db, `UPDATE strand_revisions SET session_id = NULL WHERE session_id = ?`, [sessionId]);
    runExec(db, `DELETE FROM chat_sessions WHERE id = ?`, [sessionId]);
  });
  txn();
}
...
export async function updateSession(
  ...
  const updatedVaultId = payload.bucketId ?? row.vault_id ?? "";
...
/**
 * Note slugs most recently created/updated from chat in this bucket (any session), for context retrieval.
 */
export async function listRecentVaultTouchedSlugs(
  vaultId: string,
  limit: number
): Promise<string[]> {
  const db = getDatabase();
  const cap = Math.min(Math.max(limit, 1), 32);
  const rows = allRows<{ slug: string }>(
    db,
    `SELECT DISTINCT REPLACE(wo.file, '.md', '') AS slug
     FROM wiki_ops wo
     INNER JOIN chat_sessions cs ON cs.id = wo.session_id
     WHERE cs.vault_id = ?
       AND wo.action IN ('create', 'rewrite', 'append')
...
export async function listWikiTouchSessionsForBucket(
  bucketId: string
...
/** Latest extraction/write lineage for a bucket markdown file (e.g. `slug.md`). */
...
export function recordStrandRevision(input: {
  ...
  runExec(
    ...
    [id, input.bucketId, input.file, input.bodyUtf8, hash, now, input.actor, sessionId]
  );
  ...
  if (count > STRAND_REVISION_MAX_PER_FILE) {
    ...
    runExec(
      ...
      [input.bucketId, input.file, excess]
    );
  }
}
...
export async function listStrandRevisionsForFile(
  ...
  const rows = allRows<{
    ...
    `SELECT sr.id,
     ...
     LIMIT ?`,
    [bucketId, file, STRAND_REVISION_MAX_PER_FILE]
  );
...
export async function listMemoryItems(bucketId: string): Promise<MemoryItem[]> {
  const db = getDatabase();
...
export async function createThought(input: CreateThoughtInput): Promise<ThoughtRecord> {
  ...
  const row = firstRow<ThoughtRow>(
    ...
    [id, input.bucketId, content, now, now, sourceType, input.backingNoteSlug ?? null]
  );
...
export async function listThoughtsForBucket(bucketId: string, limit = 300): Promise<ThoughtRecord[]> {
  const db = getDatabase();
...
export async function listNoteEmbeddings(bucketId: string): Promise<StoredNoteEmbedding[]> {
  const db = getDatabase();
...
export async function deleteMissingNoteEmbeddings(
  ...
  if (existingSlugs.length === 0) {
    runExec(db, `DELETE FROM note_embeddings WHERE vault_id = ?`, [bucketId]);
    return;
  }
