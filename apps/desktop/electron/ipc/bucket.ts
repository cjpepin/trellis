...
import type {
  ...
  SaveNoteInput,
  SaveNoteResult,
  BucketImportNoteImageResult,
  BucketReadNoteAssetDataUrlInput,
  BucketSnapshot,
  WikiNote
} from "./types";
import { ipcChannels, TRELLIS_DEFAULT_CHAT_IMAGE_NOTE_SLUG } from "./types";
import { resolveChatImageBytes } from "../lib/resolveChatImageBytes";
import { recordStrandRevision } from "../lib/database";
import { rebuildBucketEmbeddings, syncNoteEmbeddings } from "../lib/retrieval/index";
...
function resolveNoteAssetPath(
  bucketPath: string,
  input: BucketReadNoteAssetDataUrlInput
): string | null {
  ...
  const wikiRoot = path.join(bucketPath, "wiki");
  const notePath = ensureInsideVault(bucketPath, path.join(wikiRoot, input.noteRelativePath));
...
async function importNoteImageForVault(
  ...
  bucketPath: string,
  input: { fileId: string; noteAssetsPath?: string; noteRelativePath: string; alt?: string }
): Promise<BucketImportNoteImageResult> {
  const got = await resolveChatImageBytes(getAuth, {
    fileId: input.fileId,
    noteAssetsPath: input.noteAssetsPath
  });

  if (!got) {
    if (input.noteAssetsPath?.trim()) {
      throw new Error(
        "That image could not be loaded from cloud storage. Check that you are signed in and try again."
      );
    }
    throw new Error("That image is no longer in local cache. Attach it again.");
  }

  return importNoteImageBytesForVault(bucketPath, {
    bytes: got.bytes,
...
async function importNoteImageBytesForVault(
  ...
  const assetsDir = ensureInsideVault(
    bucketPath,
    path.join(bucketPath, "wiki", ".trellis-note-assets")
  );
  await ensureDirectory(assetsDir);

  const imageAbsolutePath = ensureInsideVault(bucketPath, path.join(assetsDir, fileName));
  await fs.writeFile(imageAbsolutePath, input.bytes);

  const wikiRoot = path.join(bucketPath, "wiki");
  const noteAbsolutePath = ensureInsideVault(bucketPath, path.join(wikiRoot, input.noteRelativePath));
...
async function readNoteAssetDataUrlForVault(
  ...
  try {
    assetPath = resolveNoteAssetPath(bucketPath, input);
  } catch {
    return null;
  }
...
async function appendChatImageToNoteForVault(
  ...
  bucketPath: string,
  bucketId: string,
  input: { fileId: string; noteAssetsPath?: string; slug?: string; alt?: string }
  ...
  const mediaDirAbsolute = ensureInsideVault(
    bucketPath,
    path.join(bucketPath, "wiki", ".trellis-chat-media")
  );
  ...
  const note = await readNoteOrCreateIfMissing(bucketPath, targetSlug);
  const wikiRoot = path.join(bucketPath, "wiki");
  const noteAbsolutePath = ensureInsideVault(bucketPath, path.join(wikiRoot, note.relativePath));
  ...
  return writeNoteFile(bucketPath, bucketId, {
    bucketId,
    slug: note.slug,
...
async function ensureBucketLayout(bucketPath: string): Promise<void> {
  await ensureDirectory(path.join(bucketPath, "wiki"));
  await ensureDirectory(path.join(bucketPath, "raw"));
}

function ensureInsideVault(bucketPath: string, targetPath: string): string {
  const resolvedVault = path.resolve(bucketPath);
 ...
 */
export async function readNoteOrCreateIfMissing(bucketPath: string, slug: string): Promise<WikiNote> {
  await ensureBucketLayout(bucketPath);
  const existingPath = await findNotePathBySlug(bucketPath, slug);
  const targetPath =
    existingPath ??
    ensureInsideVault(bucketPath, path.join(bucketPath, "wiki", `${slug}.md`));
  ...
  try {
    ...
    invalidateBucketSnapshotCache(bucketPath);
  }

  return parseNote(bucketPath, targetPath);
}

export async function readNoteIfExists(bucketPath: string, slug: string): Promise<WikiNote | null> {
  await ensureBucketLayout(bucketPath);
  const existingPath = await findNotePathBySlug(bucketPath, slug);
  ...
  return parseNote(bucketPath, existingPath);
}

async function parseNote(bucketPath: string, filePath: string): Promise<WikiNote> {
...
async function walkWikiTree(
  ...
  for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
    ...
    if (entry.isDirectory()) {
      ...
      const nested = await walkWikiTree(bucketPath, entryPath, nextFolder);
    ...
    try {
      notes.push(await parseNote(bucketPath, entryPath));
    } catch (error) {
...
async function findNotePathBySlug(bucketPath: string, slug: string): Promise<string | null> {
  const notes = await readAllNotes(bucketPath);
  ...
  return ensureInsideVault(bucketPath, path.join(bucketPath, "wiki", match.relativePath));
}
...
async function ensureUniqueSlug(
  ...
  const existing = await readAllNotes(bucketPath);
...
async function readAllNotes(bucketPath: string): Promise<WikiNote[]> {
  await ensureBucketLayout(bucketPath);
  const wikiPath = path.join(bucketPath, "wiki");
  const { notes: validNotes } = await walkWikiTree(bucketPath, wikiPath);
  buildGraph(validNotes);

  return validNotes.sort((left, right) => right.updated.localeCompare(left.updated));
}

export function resolveBucket(settings: AppSettings, bucketId?: string) {
  const resolvedVault =
...
const vaultSnapshotCache = new Map<string, BucketSnapshot>();

function vaultSnapshotCacheKey(bucketPath: string, bucketId: string, bucketName: string): string {
  return `${path.resolve(bucketPath)}\0${bucketId}\0${bucketName}`;
}

/** Call after any wiki/raw mutation that should be visible to the next index/snapshot read. */
function invalidateBucketSnapshotCache(bucketPath: string): void {
...
export async function buildSnapshot(
  bucketPath: string,
  bucketId = "active-vault",
  bucketName = "Current Vault"
): Promise<BucketSnapshot> {
  const key = vaultSnapshotCacheKey(bucketPath, bucketId, bucketName);
  ...
  const notes = await readAllNotes(bucketPath);
  const { folderPaths } = await walkWikiTree(bucketPath, path.join(bucketPath, "wiki"));
...
async function writeImportedNoteAtRelativePath(
  ...
  const wikiRoot = path.join(bucketPath, "wiki");
  const targetPath = ensureInsideVault(bucketPath, path.join(wikiRoot, relativePath));
...
export async function importFromObsidianVault(
  ...
  const existingNotes = await readAllNotes(bucketPath);
  ...
  const notes = await readAllNotes(bucketPath);
  await rebuildBucketEmbeddings(bucketId, notes);

  return {
    sourcePath: resolvedSourcePath,
    targetPath: ensureInsideVault(bucketPath, path.join(bucketPath, "wiki", importRootFolder)),
...
export async function exportToObsidianVault(
  ...
  for (const note of notes) {
    const sourceFilePath = ensureInsideVault(bucketPath, path.join(wikiRoot, note.relativePath));
...
async function removeEmptyWikiDirectories(bucketPath: string, startingDirectory: string): Promise<void> {
  const wikiRoot = path.join(bucketPath, "wiki");
...
export async function writeNoteFile(
  ...
  const existingPath = input.relativePath
    ? ensureInsideVault(bucketPath, path.join(wikiRoot, input.relativePath))
    : input.slug
    ? await findNotePathBySlug(bucketPath, input.slug)
    : null;
  const existingRelativePath = existingPath ? toPosixRelative(wikiRoot, existingPath) : undefined;
  const slug =
    input.slug && input.slug.length > 0
      ? input.slug
      : await ensureUniqueSlug(bucketPath, slugifyNoteTitle(input.title), existingRelativePath);
  ...
  const targetPath = ensureInsideVault(bucketPath, path.join(wikiRoot, folderPath, `${slug}.md`));
  ...
  if (existingPath && existingPath !== targetPath) {
    ...
    await removeEmptyWikiDirectories(bucketPath, path.dirname(existingPath));
  }
  ...
  const note = await parseNote(bucketPath, targetPath);
  await syncNoteEmbeddings(bucketId, note);

  return {
    note,
    graph: (await buildSnapshot(bucketPath)).graph
  };
}
...
async function deleteNoteFile(
  ...
  await removeEmptyWikiDirectories(bucketPath, path.dirname(targetPath));
  invalidateBucketSnapshotCache(bucketPath);
  const snapshot = await buildSnapshot(bucketPath);
  await rebuildBucketEmbeddings(bucketId, await readAllNotes(bucketPath));
  return snapshot;
}
...
export async function createFolder(
  ...
  const targetPath = ensureInsideVault(
    bucketPath,
    path.join(bucketPath, "wiki", parentPath, folderName)
  );
  await ensureDirectory(targetPath);
  invalidateBucketSnapshotCache(bucketPath);
  return buildSnapshot(bucketPath);
}
...
async function renameFolder(
  ...
  await ensureBucketLayout(bucketPath);
  const sourcePath = ensureInsideVault(bucketPath, path.join(bucketPath, "wiki", input.path));
  const targetParentPath = ensureInsideVault(
    bucketPath,
    path.join(bucketPath, "wiki", normalizeFolderPath(input.parentPath))
  );
  const targetPath = ensureInsideVault(
    bucketPath,
...
async function deleteFolder(
  ...
  await ensureBucketLayout(bucketPath);
  const targetPath = ensureInsideVault(bucketPath, path.join(bucketPath, "wiki", input.path));
...
export function registerBucketIpc(
  getSettings: () => AppSettings,
  getAuthSession: () => AuthSessionSnapshot | null
): void {
  ipcMain.handle(ipcChannels.bucketListIndex, async (_event, bucketId: unknown) => {
    const resolvedVaultId = z.string().min(1).optional().parse(bucketId);
    const vault = resolveBucket(getSettings(), resolvedVaultId);
    return buildSnapshot(vault.path, vault.id, vault.name);
  });
  ipcMain.handle(ipcChannels.bucketReadNote, async (_event, payload: unknown) => {
    const parsed = noteLookupSchema.parse(payload);
    const vault = resolveBucket(getSettings(), parsed.bucketId);
    return readNoteOrCreateIfMissing(vault.path, parsed.slug);
  });
  ipcMain.handle(ipcChannels.bucketWriteNote, async (_event, input: unknown) => {
    const parsed = saveNoteSchema.parse(input);
    const vault = resolveBucket(getSettings(), parsed.bucketId);
    return writeNoteFile(vault.path, vault.id, parsed);
  });
  ipcMain.handle(ipcChannels.bucketAppendChatImage, async (_event, input: unknown) => {
    const parsed = appendChatImageSchema.parse(input);
    const vault = resolveBucket(getSettings(), parsed.bucketId);
    return appendChatImageToNoteForVault(getAuthSession, vault.path, vault.id, parsed);
  });
  ipcMain.handle(ipcChannels.bucketImportNoteImage, async (_event, input: unknown) => {
    const parsed = importNoteImageSchema.parse(input);
    const vault = resolveBucket(getSettings(), parsed.bucketId);
    return importNoteImageForVault(getAuthSession, vault.path, parsed);
  });
  ipcMain.handle(ipcChannels.bucketReadNoteAssetDataUrl, async (_event, input: unknown) => {
    const parsed = readNoteAssetDataUrlSchema.parse(input);
    const vault = resolveBucket(getSettings(), parsed.bucketId);
    return readNoteAssetDataUrlForVault(vault.path, parsed);
  });
  ipcMain.handle(ipcChannels.bucketCreateStub, async (_event, input: unknown) => {
    const parsed = createStubSchema.parse(input);
    const vault = resolveBucket(getSettings(), parsed.bucketId);
    return createStubNote(vault.path, vault.id, parsed);
  });
  ipcMain.handle(ipcChannels.bucketDeleteNote, async (_event, input: unknown) => {
    const parsed = deleteNoteSchema.parse(input);
    const vault = resolveBucket(getSettings(), parsed.bucketId);
    return deleteNoteFile(vault.path, vault.id, parsed);
  });
  ipcMain.handle(ipcChannels.bucketCreateFolder, async (_event, input: unknown) => {
    const parsed = createFolderSchema.parse(input);
    const vault = resolveBucket(getSettings(), parsed.bucketId);
    return createFolder(vault.path, parsed);
  });
  ipcMain.handle(ipcChannels.bucketRenameFolder, async (_event, input: unknown) => {
    const parsed = renameFolderSchema.parse(input);
    const vault = resolveBucket(getSettings(), parsed.bucketId);
    return renameFolder(vault.path, parsed);
  });
  ipcMain.handle(ipcChannels.bucketDeleteFolder, async (_event, input: unknown) => {
    const parsed = deleteFolderSchema.parse(input);
    const vault = resolveBucket(getSettings(), parsed.bucketId);
    return deleteFolder(vault.path, vault.id, parsed);
  });
  ipcMain.handle(ipcChannels.bucketSelectDirectory, async (_event, payload: unknown) => {
  ...
  ipcMain.handle(ipcChannels.bucketImportFromObsidian, async (_event, input: unknown) => {
    const parsed = importFromObsidianSchema.parse(input);
    const vault = resolveBucket(getSettings(), parsed.bucketId);
    return importFromObsidianVault(vault.path, vault.id, parsed);
  });
  ipcMain.handle(ipcChannels.bucketExportToObsidian, async (_event, input: unknown) => {
...
export async function saveRawSource(
  ...
  const targetPath = ensureInsideVault(
    bucketPath,
    path.join(bucketPath, "raw", safeName)
  );
