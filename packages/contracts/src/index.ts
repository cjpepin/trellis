...
import type { ReadAloudSpeedTier } from "@trellis/shared/media/readAloudSpeed";
import type { CreateThoughtInput, ThoughtRecord } from "@trellis/shared/thoughts/types";

export type { CreateThoughtInput, ThoughtRecord };
export type { ThoughtEnrichment, ThoughtSourceType, ThoughtStatus } from "@trellis/shared/thoughts/types";
...
export const ipcChannels = {
  ...
  dbDeleteSession: "db:delete:session",
  ...
  bucketListIndex: "bucket:list:index",
  bucketReadNote: "bucket:read:note",
  bucketWriteNote: "bucket:write:note",
  bucketCreateStub: "bucket:create:stub",
  bucketDeleteNote: "bucket:delete:note",
  bucketCreateFolder: "bucket:create:folder",
  bucketRenameFolder: "bucket:rename:folder",
  bucketDeleteFolder: "bucket:delete:folder",
  bucketSelectDirectory: "bucket:select:directory",
  bucketImportFromObsidian: "bucket:import:obsidian",
  bucketExportToObsidian: "bucket:export:obsidian",
  bucketAppendChatImage: "bucket:append:chat-image",
  bucketImportNoteImage: "bucket:import-note-image",
  bucketReadNoteAssetDataUrl: "bucket:read-note-asset-data-url",
  ...
  chatApplyBucketOrganize: "chat:apply-bucket-organize",
...
export interface ChatMediaArtifact {
  ...
  fileId: string;
  /**
   * When set, image bytes for this artifact live in the Supabase `note-assets` bucket at this
   * object path (first path segment = workspace id). Other devices use this for display instead
   * of the local media cache.
   */
  noteAssetsPath?: string;
  mimeType: string;
  label: string;
...
export interface BucketAppendChatImageInput {
  bucketId?: string;
  fileId: string;
  /** Supabase `note-assets` object path when the image is not in local media cache. */
  noteAssetsPath?: string;
  slug?: string;
  alt?: string;
}

export interface BucketImportNoteImageInput {
  bucketId?: string;
  fileId: string;
  /** Supabase `note-assets` object path when the image is not in local media cache. */
  noteAssetsPath?: string;
...
export interface BucketImportNoteImageResult {
  markdownPath: string;
  alt: string;
}

export interface BucketReadNoteAssetDataUrlInput {
  bucketId?: string;
  noteRelativePath: string;
  assetPath: string;
}
...
export interface ApplyBucketOrganizeInput {
  bucketId: string;
  userMessage: string;
}
...
export interface ApplyBucketOrganizeResult {
  applied: boolean;
  message: string | null;
...
export interface AppSettings {
  ...
  /**
   * When false on desktop personal workspace, Strands and chat history stay on this device
   * while hosted chat can still use your plan (trial/BYOK/Pro). Default true.
   */
...
export interface DatabaseBridge {
  ...
  createSession: (payload: { model: ChatModel; bucketId: string }) => Promise<ChatSessionSummary>;
  ...
  listWikiTouchSessions: (bucketId: string) => Promise<WikiTouchSessionSummary[]>;
  getStrandProvenanceForFile: (input: {
    bucketId: string;
    fileName: string;
  }) => Promise<StrandProvenanceSnapshot | null>;
  listStrandRevisions: (input: { bucketId: string; file: string }) => Promise<StrandRevisionSummary[]>;
  getStrandRevisionBody: (input: {
    bucketId: string;
    revisionId: string;
  }) => Promise<{ body: string } | null>;
  createThought: (input: CreateThoughtInput) => Promise<ThoughtRecord>;
  listThoughts: (bucketId: string) => Promise<ThoughtRecord[]>;
  getThought: (thoughtId: string) => Promise<ThoughtRecord | null>;
  retryThoughtEnrichment: (thoughtId: string) => Promise<void>;
}

export interface BucketBridge {
  listIndex: (bucketId?: string) => Promise<BucketSnapshot>;
  readNote: (slug: string, bucketId?: string) => Promise<WikiNote>;
  writeNote: (input: SaveNoteInput) => Promise<SaveNoteResult>;
  appendChatImageToNote: (input: BucketAppendChatImageInput) => Promise<SaveNoteResult>;
  importNoteImage: (input: BucketImportNoteImageInput) => Promise<BucketImportNoteImageResult>;
  readNoteAssetDataUrl: (input: BucketReadNoteAssetDataUrlInput) => Promise<string | null>;
  createStub: (input: CreateStubInput) => Promise<SaveNoteResult>;
  deleteNote: (input: DeleteNoteInput) => Promise<BucketSnapshot>;
  createFolder: (input: CreateFolderInput) => Promise<BucketSnapshot>;
  renameFolder: (input: RenameFolderInput) => Promise<BucketSnapshot>;
  deleteFolder: (input: DeleteFolderInput) => Promise<BucketSnapshot>;
...
export interface RetrievalBridge {
  ...
  rebuildIndex: (bucketId?: string) => Promise<RetrievalRebuildResult>;
}
...
export interface ChatBridge {
  ...
  applyBucketOrganize: (input: ApplyBucketOrganizeInput) => Promise<ApplyBucketOrganizeResult>;
